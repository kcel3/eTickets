...
namespace eTickets.Controllers
{
    public class ProducersController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(int id,[Bind("ProfilePictureURL,FullName,Biography")] Producer producer)
        {
            if (!ModelState.IsValid) return View(producer);
            if (id == producer.Id)
            {
        ...
        public async Task<IActionResult> Delete(int id, [Bind("ProfilePictureURL,FullName,Biography")] Producer producer)
        {
            var producerDetails=await _service.GetByIdAsync(id);
            if (producerDetails == null) return View("NotFound");
